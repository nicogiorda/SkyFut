...
INSERT INTO torneo (nombre, fecha_inicio, estado)
SELECT 'SkyFut Champions 2026', '2026-10-01', 'EN_CURSO'
WHERE NOT EXISTS (
    SELECT 1
    FROM torneo
    WHERE nombre = 'SkyFut Champions 2026'
);

INSERT INTO fase (id_torneo, nombre, orden, completada)
SELECT id, 'Octavos de Final', 1, 0
FROM torneo
WHERE nombre = 'SkyFut Champions 2026'
  AND NOT EXISTS (
      SELECT 1
      FROM fase
      WHERE fase.id_torneo = torneo.id
        AND fase.nombre = 'Octavos de Final'
  );

CREATE TEMP TABLE IF NOT EXISTS seed_partido (
    local TEXT NOT NULL,
    visitante TEXT NOT NULL
);

DELETE FROM seed_partido;

INSERT INTO seed_partido (local, visitante)
VALUES
    ('Real Madrid', 'Benfica'),
...
SELECT
    ...
    'NO_INICIADO',
    local.id_tactica,
    visitante.id_tactica
FROM seed_partido sp
JOIN equipo local ON local.nombre = sp.local
JOIN equipo visitante ON visitante.nombre = sp.visitante
JOIN torneo t ON t.nombre = 'SkyFut Champions 2026'
JOIN fase f ON f.id_torneo = t.id AND f.nombre = 'Octavos de Final'
WHERE NOT EXISTS (
    SELECT 1
    FROM partido existente
    WHERE existente.id_fase = f.id
      AND existente.id_equipo_local = local.id
      AND existente.id_equipo_visit = visitante.id
);

DROP TABLE seed_partido;
